...
window = tk.Tk()
window.title('Stemlab Attendance System V-001')
...
title = tk.Label(window,text="Stemlab Attendance System V-001",bd=10,relief=tk.GROOVE,font=("times new roman",40),bg="lavender",fg="black")
...
combo_search['values']=("Admin", "Intern")
...
combo_search['values']=('Admin Dept','IT Dept', 'Productivity Dept','Communication Dept','Intern Group A','Intern Group B')
combo_search.place(x=250,y=250)

ttk.Label(window, text = "Session",background="lavender", foreground ="black",font = ("Times New Roman", 15)).place(x=100,y=300)
combo_search=ttk.Combobox(window,textvariable=period,width=10,font=("times new roman",13),state='readonly')
combo_search['values']=('9am-1pm (Intern)','2pm-5pm', '9am-8pm(Admin)')
combo_search.place(x=250,y=300)

def checkk():
    if( branch.get() and period.get() and sec.get()):
        window.destroy()
    else:
